...
from .angle import from_rad, wrap_180
from .utc import UTC
import fastfix.angle as angle
...
class Location(object):
    ...
    def longitude_deg(self):
        return wrap_180(self.lon.to_degrees())
    ...
    def get_XYZ(self, e, n, u):
        ...
        d_lon = from_rad((e/self.R_EARTH)*self.lat.sin())
        d_lat = from_rad(n/self.R_EARTH)
    ...
    def GST(utc_date):
        ...
        dphi = -0.000319*math.sin(np.radians(omega)) - 0.000024*math.sin(np.radians(2.0*L))
        eqeq = dphi * math.cos(np.radians(epsilon))
