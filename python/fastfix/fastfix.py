...
def doppler_fmap(
    lat, lon, delta_f, x_sv, v_sv  # Center frequency offset  # Satellite Positions ECEF
):  # Satellite Velocities ECEF
    """
...
def phase_fmap(
    svs, lat, lon, alt, offset, sow  # Phase offset  # Absolute GPS second of week
):

    rp = Location(from_dms(lat), from_dms(lon), alt)
    # Reciever location in ECEF
    r_0 = rp.get_ecef()
...
def doppler_fix(acq, t0, ephs):
    ...
    return {"lat": r0.x[0], "lon": r0.x[1], "delta_f": r0.x[2]}
...
def phase_fix(acq, r0, t0, ephs):
    ...
    if True:
        ...
        minimizer_kwargs = {
            ...
            "tol": 1e-5,
            "options": {"maxcor": 48},
            "args": (sats, xmax, meas_phase),
        }
        r0 = optimize.basinhopping(
            phase_f_opt,
            x0,
            niter=100,
            T=100,
