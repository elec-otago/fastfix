...
def acquire_all(rx, fs, fc0, searchBand, plot_corr=False):	
    ...
    sampling_period, samples_per_ms, samples_per_chip, samples_per_chunk, numberOfFrqBins = setup_fftw(fs, fc0, searchBand)
    ...
    if (threads == 1):
        ...
        for i in range(0,svMax):
            sv = i + 1
            resultList.append(p.apply_async(acquire, (rx, fs, fc0, searchBand, sv, plot_corr)))
        ...
        for sv in range(0,svMax):
            [p, strength[sv], phase[sv], freq[sv]] = acquire(rx, fs, fc0, searchBand, sv+1, plot_corr);
...
def decode(filename, iq):
    with open(filename, "rb") as f:
        ...
        rtc_counter_value, sample_count, sample_bytes, sample_ms, sampling_rate, checksum = s.unpack(bytes_read[0:24])
...
class TestAcquisition(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_ms, self.sampling_rate, self.checksum, self.dat = decode(self.raw_file)
    ...
    def test_zsimple(self):
        tag_acquire(self.filename, fs=self.fs, resolution=1, epochs=4, fc0=self.fc0, return_filename='results.out')
